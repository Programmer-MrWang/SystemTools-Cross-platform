...
public class ShutdownAction(ILogger<ShutdownAction> logger) : ActionBase<ShutdownSettings>
{
    ...
    protected override async Task OnInvoke()
    {
        ...
        {
            _logger.LogWarning("计时关机预检未通过：当前环境不支持（平台能力门），跳过执行。");
            await NotifyDegradedAsync("计时关机", "计时关机在当前环境不可用，已跳过执行");
            await base.OnInvoke();
            return;
        }

        var result = SystemPowerCommand.RunTimedShutdown(Settings.Seconds);
        if (result.ExitCode != PowerCommandResult.Ok)
        {
            _logger.LogError("计时关机命令未被执行（exit={ExitCode}，detail={Detail}）。", result.ExitCode, result.Detail);
            await NotifyDegradedAsync("计时关机", "计时关机命令未被执行");
            await base.OnInvoke();
            return;
        }

        _logger.LogInformation("已执行计时关机（{Seconds} 秒后；{PlanKind}）。", Settings.Seconds,
            OperatingSystem.IsWindows() ? "系统级计划" : "应用内计划，宿主退出即取消");
